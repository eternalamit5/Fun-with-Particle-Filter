...
%In this tutorial, The  Nation's Army Jet has just run into the mysterious Enemy Jet.
...
x_R = 1; % Noise covariance in the measurement (i.e. the Enemy Jet creates complex illusions in its trail!)
T = 75; % duration the chase (i.e. number of iterations).
N = 200; % The number of particles the system generates. The larger this is, the better your approximation, but the more computation you need.
...
%the functions used by the Enemy Jet are:
...
for t = 1:T
    ...
    %position (i.e. update the Enemy Jets position with the non linear function
    %and update from this position what the chasing Army Jet see confounded
    %by the Enemy Jets illusions.
    ...
    for i = 1:N
        ...
        %the Enemy Jet might be), run each of these particles through the state
        ...
        %gaussian)
        P_w(i) = (1/sqrt(2*pi*x_R)) * exp(-(z - z_update(i))^2/(2*x_R)); % Given the z_update, what is the probability of getting the z or actual value
    end
...
xlabel('time step'); ylabel('Enemy Jet flight position');
